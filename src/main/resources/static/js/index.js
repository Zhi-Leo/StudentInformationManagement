...
function loadSharedComponents() {
	// 1. 加载导航栏
	const navbarHtml = `
        <nav class="bg-white/50 shadow-md navHTML">
            <div class="mx-auto px-4 sm:px-6 lg:px-0">
                <div class="flex flex-col justify-between h-16">
                    ...
                    <div class="flex items-center flex-col">
                        ...
                        <button id="studentsTab" class="px-4 py-2 text-primary font-medium border-b-2 hover:text-[#3B82F6]">
	...
	// 2. 加载页脚
	const footerHtml = `
    <footer class="bg-gray-800 text-white py-6 flex footer-fixed">
...
window.confirmDelete = function (entityType, id, name) {
	...
	document.getElementById(
		"deleteMessage"
	).textContent = `你确定要删除${entityName}"${name}"吗？此操作不可撤销。`;
...
window.showNotification = function (type, title, message) {
	...
	const styles = {
		success: {
			...
			bg: "bg-green-50 text-green-700 border-l-4 border-green-400",
		},
		error: {
			icon: "fa-exclamation-circle",
			bg: "bg-red-50 text-red-700 border-l-4 border-red-400",
		},
		warning: {
			icon: "fa-exclamation-triangle",
			bg: "bg-yellow-50 text-yellow-700 border-l-4 border-yellow-400",
		},
	};
