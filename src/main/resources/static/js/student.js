	...
	return fetch("index.html")
		...
		.then((html) => {
			...
			if (tpl && document.getElementById("navbarContainer")) {
				const navbarHtml = tpl.innerHTML.replace(
...
function addStudent() {
	...
	if (!name)
		return window.showNotification("error", "错误", "学生姓名不能为空");
	if (isNaN(age) || age < 5 || age > 100)
		return window.showNotification("error", "错误", "年龄需为5-100之间的数字");
	if (isNaN(grade) || grade < 0 || grade > 100)
		return window.showNotification("error", "错误", "成绩需为0-100之间的数字");
...
function filterStudents() {
	const searchTerm =
		document.getElementById("studentSearch")?.value.toLowerCase().trim() || "";
...
function deleteStudent(studentId) {
	const allDeleteBtns = document.querySelectorAll(
		"#studentsBody .text-red-600"
	);
	...
	fetch(`/api/students/${studentId}`, {
		...
		.then(() => {
			originalStudents = originalStudents.filter(
